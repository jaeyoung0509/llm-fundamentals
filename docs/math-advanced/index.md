...
1. 식 안에서 `sin`, `rank`, `KL`, `Jacobian`처럼 눈에 띄는 수학 신호를 먼저 표시한다.
2. 각 신호가 "위치", "방향", "민감도", "분포 차이" 중 무엇을 다루는지 한 문장으로 적는다.
3. 식을 작은 숫자 예시로 줄여 본다.
4. 마지막에만 PyTorch에서 어느 연산인지 찾는다.

이 순서를 지키면 갑자기 어려운 정의 속으로 떨어지는 느낌이 줄어든다.
...
예를 들어 `CE(p, q) = H(p) + KL(p || q)`를 처음 본다면 세 기호를 한 번에 읽지 않아도 된다.

- `CE`: 지금 실제로 최적화하는 손실
- `H(p)`: 정답 분포 자체의 난이도
- `KL(p || q)`: 모델이 추가로 틀린 정도

이렇게 잘라 읽는 습관이 심화 수학 전체에서 계속 통한다.
