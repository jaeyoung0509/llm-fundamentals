...
SVD를 제일 쉽게 말하면 "복잡한 행렬을 중요한 방향 몇 개와 그 강도로 요약하는 방법"이다. 거대한 행렬도 실제로는 핵심 방향 몇 개만 강하게 쓰는 경우가 많다.

### 개발자 관점에서 다시 읽기

LoRA나 압축을 볼 때는 "모든 가중치를 다 바꾸지 말고 중요한 방향만 손보자"로 이해하면 된다. SVD는 바로 그 중요한 방향을 찾는 데 쓰는 언어다.
...
`X = [[5, 0], [0, 1]]` 같은 아주 단순한 행렬을 보자.

- 첫 번째 방향은 크기 `5`로 강하게 늘린다.
- 두 번째 방향은 크기 `1`만큼만 남긴다.
- 즉, 이 행렬은 사실상 첫 번째 방향을 훨씬 더 중요하게 취급한다.

여기서 rank-1 근사를 하면 대충 "첫 번째 방향만 남기고 두 번째 약한 방향은 버리자"가 된다. SVD를 처음 읽을 때는 이렇게 "방향별 중요도 표"로 보는 편이 가장 쉽다.
