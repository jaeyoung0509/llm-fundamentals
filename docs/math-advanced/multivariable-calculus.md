...
한 변수 미분이 "한 방향으로 조금 움직이면 어떻게 되나"라면, 다변수 미적분은 "수많은 방향이 동시에 움직이면 어떻게 되나"를 보는 수학이다. 딥러닝은 파라미터가 매우 많기 때문에 이 언어가 자연스럽다.

### 개발자 관점에서 다시 읽기

gradient는 이미 쓰고 있다. Jacobian은 "입력을 조금 바꾸면 출력 여러 개가 얼마나 흔들리나", Hessian은 "loss 지형이 얼마나 날카롭나"를 보는 확장판이라고 보면 된다.
...
`L(w_1, w_2) = w_1^2 + 4w_2^2`를 보자.

- `w_1 = 1`, `w_2 = 1`에서 gradient는 `[2, 8]`이다.
- 즉, 같은 1만큼 움직여도 `w_2` 방향이 훨씬 더 가파르다.
- Hessian은 대각선 기준으로 대략 `[2, 8]`의 곡률을 가진다.

이 한 예시만으로도 "gradient는 지금 어느 쪽으로 내려갈지", "Hessian은 어느 축이 더 날카로운지"를 나눠 읽을 수 있다.
